...
MDP = presentation_hint_task_gen_model_correct(Gen_model);
...
for t = 1:T % loop over time points  
        ...
        for Ni = 1:NumIterations % number of iterations of message passing  
            ...
            lnAo = zeros(size(state_posterior{factor})); % initialise matrix containing the log likelihood of observations
                for tau = 1:T % loop over tau
                    ...
                    if tau == 1 % first tau
                        lnD = nat_log(d{factor}); % TODO forward message from prior d
    ...
    for ni = 1:Ni 
        ...
        policy_posteriors(:,t) = exp(log(E) + gamma(t)*G(:,t) - F(:,t))/sum(exp(log(E) + gamma(t)*G(:,t) - F(:,t))); % posterior over policies TODO
    ...
    if t < T
        ...
        action_posterior_intermediate(:) = (exp(alpha*log(action_posterior_intermediate(:)))/sum(exp(alpha*log(action_posterior_intermediate(:))))); % TODO
...
if isfield(MDP,'d')
    for factor = 1:NumFactors
        i = MDP.d{factor} > 0;
        MDP.d{factor}(i) = (1-omega)*(MDP.d{factor}(i)-MDP.d_0{factor}(i)) + MDP.d_0{factor}(i) + eta*BMA_states{factor}(i,1); %TODO
